<?php $this->setPageTitle(LANG_ZAUTH_VERIFY_EMAIL); ?>

<h1><?php echo LANG_ZAUTH_VERIFY_EMAIL; ?></h1>

<?php
    $this->renderForm($form, $data, array(
        'submit' => array('title' => LANG_CONTINUE),
        'cancel' => array(
            'title' => LANG_ZAUTH_CLEAN_REG_EMAIL,
            'show' => ($reg_email ? true : false),
            'href' => $this->href_to('finish').'?clean_email=1'
        ),
        'action' => '',
        'method' => 'post',
    ), $errors);
...
<script>
    ...
    <?php if(!$errors){ ?>
        $(function(){
            vTimer.init();
            if($('#email_token').val().length == 32){
                $('#email_token').closest('form').find('.button-submit').trigger('click');
            }
        });
    <?php } ?>
</script>
